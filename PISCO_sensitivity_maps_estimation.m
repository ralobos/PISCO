...
function [senseMaps, eigenValues] = PISCO_sensitivity_maps_estimation(kCal, dim_sens, varargin)
    ...
    if flag_3D == 0
        U = utils.nullspace_vectors_C_matrix_2D(kCal, p.Results.tau, p.Results.threshold, p.Results.kernel_shape, p.Results.FFT_nullspace_C_calculation, p.Results.sketched_SVD, p.Results.sketch_dim, p.Results.visualize_C_matrix_sv);
    else
    ...
    if flag_3D == 0
        G = G_matrices_2D(kCal, dim_sens(1), dim_sens(2), p.Results.tau, U, p.Results.kernel_shape, p.Results.FFT_interpolation, p.Results.interp_zp, p.Results.sketched_SVD);
    else
    ...
    if flag_3D == 0
        ...
        dim_sens(1), dim_sens(2), G, patchSize, ...
